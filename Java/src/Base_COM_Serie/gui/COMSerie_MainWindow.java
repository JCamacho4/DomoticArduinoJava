...
public class COMSerie_MainWindow {
	...
	public COMSerie_MainWindow() {
		...
		JRadioButtonMenuItem Persiana = new JRadioButtonMenuItem("Persiana");
		Persiana.addActionListener(Select_Practica);
		mnPrctica10.add(Persiana);
		
		JRadioButtonMenuItem Garaje = new JRadioButtonMenuItem("Garaje");
		Garaje.addActionListener(Select_Practica);
		mnPrctica10.add(Garaje);
		...
		practicaSel.add(Persiana);
		practicaSel.add(Garaje);
		...
		for (int i = 0, n = practicaSel.size(); i < n; i++) {
			if(practicaSel.get(i).equals(Source)) {
				...
				ModBus_Communications.writeSingleRegister(slaveAdd, MB_Registers.MB_Analog_Output_Holding.MB_PRACT.getReg(), 
						Integer.toString(MB_Registers.SELPRACT[i]) , sn_Transport);
			}
		}
...
//			serialServer.serialConsole = null;
	...
	private void Modbus_Regular_Call(){
		Timer timer = new Timer (ConstantesApp.REGULAR_CALL, new ActionListener ()
		{
			public void actionPerformed(ActionEvent e)
		    {	
